...
var useGeolocationState = function (options) {
    ...
    var _b = react_1.useState(false), isRetrieving = _b[0], setRetrieving = _b[1];
    var _c = useGeolocationEvents_1.default(options), isSupported = _c.isSupported, onChange = _c.onChange;
    var savePosition = react_1.useCallback(function () {
        if (position === null) {
            setRetrieving(true);
            navigator.geolocation.getCurrentPosition(function (nextPosition) {
                if (!isSamePosition_1.default(position, nextPosition)) {
                    setPosition(makePositionObject_1.default(nextPosition));
                    setRetrieving(false);
                }
            });
        }
    }, [position]);
    react_1.useEffect(savePosition, [position]);
