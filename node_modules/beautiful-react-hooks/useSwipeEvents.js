 ...
 */
var useSilentSwipeState = function (targetRef, options, onSwipeStart, onSwipeMove, onSwipeEnd) {
    if (targetRef === void 0) { targetRef = null; }
    if (options === void 0) { options = defaultOptions; }
    var startingPointRef = react_1.useRef([-1, -1]);
    var directionRef = react_1.useRef(null);
    var isDraggingRef = react_1.useRef(false);
    var alphaRef = react_1.useRef([]);
    var opts = __assign(__assign({}, defaultOptions), (options || {}));
    var _a = useMouseEvents_1.default(targetRef), onMouseDown = _a.onMouseDown, onMouseMove = _a.onMouseMove, onMouseLeave = _a.onMouseLeave, onMouseUp = _a.onMouseUp;
    var _b = useTouchEvents_1.default(targetRef), onTouchStart = _b.onTouchStart, onTouchMove = _b.onTouchMove, onTouchEnd = _b.onTouchEnd, onTouchCancel = _b.onTouchCancel;
    var _c = react_1.useState(), state = _c[0], setState = _c[1];
    var startSwipe = function (event) {
        var _a = swipeUtils_1.getPointerCoordinates(event), clientX = _a[0], clientY = _a[1];
    ...
    var continueSwipe = function (event) {
        ...
        if (startingPointRef.current[0] !== -1 && startingPointRef.current[1] !== -1) {
            ...
            if (Math.abs(alpha[0]) > opts.threshold || Math.abs(alpha[1]) > opts.threshold) {
                ...
                directionRef.current = swipeUtils_1.getDirection([clientX, clientY], startingPointRef.current, alpha);
 ...
 * @param options
 */
var useSwipeEvents = function (targetRef, options) {
    if (targetRef === void 0) { targetRef = null; }
    if (options === void 0) { options = defaultOptions; }
    var opts = __assign(__assign({}, defaultOptions), (options || {}));
    var _a = useHandlerSetterRef_1.default(), onSwipeLeft = _a[0], setOnSwipeLeft = _a[1];
    var _b = useHandlerSetterRef_1.default(), onSwipeRight = _b[0], setOnSwipeRight = _b[1];
    var _c = useHandlerSetterRef_1.default(), onSwipeUp = _c[0], setOnSwipeUp = _c[1];
    var _d = useHandlerSetterRef_1.default(), onSwipeDown = _d[0], setOnSwipeDown = _d[1];
    var _e = useHandlerSetterRef_1.default(), onSwipeStart = _e[0], setOnSwipeStart = _e[1];
    var _f = useHandlerSetterRef_1.default(), onSwipeMove = _f[0], setOnSwipeMove = _f[1];
    var _g = useHandlerSetterRef_1.default(), onSwipeEnd = _g[0], setOnSwipeEnd = _g[1];
