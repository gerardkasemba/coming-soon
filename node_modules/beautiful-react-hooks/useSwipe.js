...
var useSwipe = function (targetRef, options) {
    ...
    var _b = useMouseEvents_1.default(targetRef), onMouseDown = _b.onMouseDown, onMouseMove = _b.onMouseMove, onMouseLeave = _b.onMouseLeave, onMouseUp = _b.onMouseUp;
    var _c = useTouchEvents_1.default(targetRef), onTouchStart = _c.onTouchStart, onTouchMove = _c.onTouchMove, onTouchEnd = _c.onTouchEnd, onTouchCancel = _c.onTouchCancel;
        ...
        if (isDraggingRef.current || (startingPointRef.current[0] !== -1 && startingPointRef.current[1] !== -1)) {
            ...
            if (opts.direction === 'both' && (Math.abs(alpha[0]) > opts.threshold || Math.abs(alpha[1]) > opts.threshold)) {
                ...
                var nextState = {
                    ...
                    count: state.count,
                    swiping: true,
                    direction: swipeUtils_1.getDirection([clientX, clientY], startingPointRef.current, alpha),
                };
            ...
            if (opts.direction === 'horizontal' && Math.abs(alpha[0]) > opts.threshold) {
                ...
                var nextState = {
                    ...
                    count: state.count,
                    swiping: true,
                    direction: swipeUtils_1.getHorizontalDirection(alpha[0]),
                };
                if (!isEqual(nextState, state)) {
                    setState(nextState);
                }
            ...
            if (opts.direction === 'vertical' && Math.abs(alpha[1]) > opts.threshold) {
                ...
                var nextState = {
                    ...
                    count: state.count,
                    swiping: true,
                    direction: swipeUtils_1.getVerticalDirection(alpha[1]),
                };
                if (!isEqual(nextState, state)) {
                    setState(nextState);
                }
